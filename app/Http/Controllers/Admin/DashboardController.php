...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['auth_user'] = getAuthUser('web');
        if ($data['auth_user']->hasRole('Teacher')) {
            $data['class_routines'] = $this->getClassRoutines();
            $data['event_rows'] = Event::where('status', '1')->orderBy('id', 'asc')->get();
