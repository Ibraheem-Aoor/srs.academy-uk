...
class SubjectMustBeOfferedWithSingleEnrollmentSession implements Rule
{

    protected $student , $session , $course;
    /**
     * Create a new rule instance.
     *
     * @return void
     */
    public function __construct($student_id  , $session_id)
    {
        $this->student = Student::findOrFail($student_id);
        $this->session = Session::query()->findOrFail($session_id);
    }
    ...
    {
        $subjects = $value;
        $needed_session_enrolls = EnrollSubject::query()->where('session_id', $this->session->id)->get();
        $offered_subjects = [];
    ...
    {
        return __('courses_must_be_offered_to_enrollment_session' , ['session_title' => $this->session->title , 'course' => $this->course->title]);
    }
}
