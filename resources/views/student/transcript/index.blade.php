                    ...
                    <div class="card">
                        ...
                        <ul class="list-group list-group-flush">
                            @if (isset($row->student_id))
                                <li class="list-group-item"><mark class="text-primary">{{ __('field_student_id') }} :</mark>
                                    {{ $row->student_id }}</li>
                            @endif
                            <li class="list-group-item"><mark class="text-primary">{{ __('field_name') }} :</mark>
                                {{ $row->first_name }} {{ $row->last_name }}</li>

                            <li class="list-group-item"><mark class="text-primary">{{ __('field_batch') }} :</mark>
                                {{ $row->batch->title ?? '' }}</li>
                            <li class="list-group-item"><mark class="text-primary">{{ __('field_program') }} :</mark>
                                {{ $row->program->title ?? '' }}</li>
                                        ...
                                        @foreach ($grades as $grade)
                                            @if ($marks_per >= $grade->min_mark && $marks_per <= $grade->max_mark)
                                                @php
                                                    if ($grade->point > 0) {
                                                        $total_cgpa =
                                                            $total_cgpa + $grade->point * $subject->credit_hour;
                                                        // $total_cgpa = $total_cgpa + $grade->point;
                        ...
                        <li class="list-group-item"><mark class="text-primary">{{ __('field_total_credit_hour') }}
                                :</mark> {{ round($total_credits, 2) }}</li>

                        <li class="list-group-item"><mark class="text-primary">{{ __('field_cumulative_gpa') }}
                                :</mark>
                            @php
                                if ($total_credits <= 0) {
                                    $total_credits = 1;
                                            ...
                                            @if ($semester_item[1] == $item->semester->title && $semester_item[0] == $item->session->title)
                                                ...
                                                @foreach ($item->subjects as $subject)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{ $subject->title }}
                                                            ({{ \App\Models\ProgramSubject::query()->where('subject_id', $subject->id)->where('program_id', $row->program->id)->first()?->subjectType?->title }})
                                                        </td>
                                                        ...
                                                        <td>
                                                            ...
                                                            @foreach ($grades as $grade)
                                                                @if ($marks_per >= $grade->min_mark && $marks_per <= $grade->max_mark)
                                                                    ...
                                                                    @php
                                                                        $semester_cgpa =
                                                                            // $semester_cgpa + $grade->point * $subject->credit_hour;
                                                                            $semester_cgpa + $grade->point;
