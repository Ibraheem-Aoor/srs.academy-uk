                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item">
                                <span class="f-w-500"><i
                                        class="far fa-calendar-alt m-r-10"></i>{{ __('field_apply_date') }} : </span>
                            ...
                            <li class="list-group-item border-bottom-0">
                                <span class="f-w-500"><i class="far fa-question-circle m-r-10"></i>{{ __('field_status') }}
                                    : </span>
                                <span class="float-end">
                                    @if ($row->status == 1)
                                    ...
                                    <div class="col-md-4">
                                        @if (field('application_school_info')->status == 1)
                                            <fieldset class="row gx-2 scheduler-border">
                                                ...
                                                <p><mark class="text-primary">{{ __('field_school_name') }}:</mark>
                                                    {{ $row->school_name }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_exam_id') }}:</mark>
                                                    {{ $row->school_exam_id }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_graduation_year') }}:</mark>
                                                    {{ $row->school_graduation_year }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_graduation_point') }}:</mark>
                                                    {{ $row->school_graduation_point }}</p>
                                                <hr />
                                            </fieldset>
                                        @endif
                                        ...
                                        @if (field('application_collage_info')->status == 1)
                                            <fieldset class="row gx-2 scheduler-border">
                                                ...
                                                <p><mark class="text-primary">{{ __('field_collage_name') }}:</mark>
                                                    {{ $row->collage_name }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_exam_id') }}:</mark>
                                                    {{ $row->collage_exam_id }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_graduation_year') }}:</mark>
                                                    {{ $row->collage_graduation_year }}</p>
                                                <hr />
                                                <p><mark class="text-primary">{{ __('field_graduation_point') }}:</mark>
                                                    {{ $row->collage_graduation_point }}</p>
                                                <hr />
                                            </fieldset>
                                        @endif
                                        <fieldset class="row gx-2 scheduler-border attachmers">
                                            <legend>{{ __('attachments') }}</legend>
                                            @isset($row->grade_transcripts)
                                                <p><mark class="text-primary">{{ __('grade_transcripts') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'grade_transcripts/' . $row->grade_transcripts]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->previous_certificates)
                                                <p><mark class="text-primary">{{ __('previous_certificates') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'previous_certificates/' . $row->previous_certificates]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->en_certificate)
                                                <p><mark class="text-primary">{{ __('en_certificate') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'en_certificate/' . $row->en_certificate]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->certificate_of_recommendation)
                                                <p><mark
                                                        class="text-primary">{{ __('certificate_of_recommendation') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'certificate_of_recommendation/' . $row->certificate_of_recommendation]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset

                                            @isset($row->english_certificate)
                                                <p><mark class="text-primary">{{ __('english_certificate') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'english_certificate/' . $row->english_certificate]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->letter_of_intrest)
                                                <p><mark class="text-primary">{{ __('letter_of_intrest') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'letter_of_intrest/' . $row->letter_of_intrest]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->id_or_passport)
                                                <p><mark class="text-primary">{{ __('id_or_passport') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'id_or_passport/' . $row->id_or_passport]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
                                            @isset($row->proof_master_in_en)
                                                <p><mark class="text-primary">{{ __('proof_master_in_en') }}:</mark><a
                                                        href="{{ route('admin.download', ['file' => $application_files_path . 'proof_master_in_en/' . $row->proof_master_in_en]) }}"
                                                        class="btn btn-sm btn-soft-success"><i class="fa fa-download"></i></a>
                                                </p>
                                                <hr />
                                            @endisset
