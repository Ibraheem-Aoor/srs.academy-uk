                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            @can($access . '-import')
                                <a href="{{ route($route . '.import') }}" class="btn btn-dark btn-sm float-right"><i
                            ...
                            <div class="card-header">
                                <div class="form-group d-inline">
                                    <div class="radio radio-primary d-inline">
                                        ...
                                        <label for="attendance-p" class="cr">{{ __('all') }}
                                            {{ __('attendance_present') }}</label>
                                    </div>
                                </div>
                                <div class="form-group d-inline">
                                    <div class="radio radio-danger d-inline">
                                        <input type="radio" name="all_check" id="attendance-a" class="all_absent">
                                        <label for="attendance-a" class="cr">{{ __('all') }}
                                            {{ __('attendance_absent') }}</label>
                                    </div>
                                </div>
                                <div class="clearfix"></div>
                            </div>

                            <form class="needs-validation" novalidate action="{{ route($route . '.store') }}"
                                    ...
                                    <div class="row">
                                        <div class="form-group col-sm-6 col-md-3">
                                            ...
                                            <input type="date" class="form-control date" name="date" id="date"
                                                value="{{ $check_data->date ?? '' }}" required>
                                                ...
                                                @foreach ($rows as $key => $row)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <div class="form-group d-inline">
                                                                <div class="radio radio-primary d-inline">
                                                                    <input class="c-present" type="radio"
                                                                        data_id="{{ $row->id }}"name="attendances-{{ $key }}"
                                                                        id="attendance-p-{{ $key }}"
                                                                        value="1" required
                                                                        @if (isset($attendances)) @foreach ($attendances as $attendance)
                                                    @if ($attendance->student_enroll_id == $row->id && $attendance->attendance == 1)
                                                        checked @endif
                                                                        @endforeach
                                                @endif
                                                >
                                                <label for="attendance-p-{{ $key }}"
                                                    class="cr">{{ __('attendance_present') }}</label>
                                    </div>
                                </div>
                                <div class="form-group d-inline">
                                    <div class="radio radio-danger d-inline">
                                        <input class="c-absent" type="radio"
                                            data_id="{{ $row->id }}"name="attendances-{{ $key }}"
                                            id="attendance-a-{{ $key }}" value="2" required
                                            @if (isset($attendances)) @foreach ($attendances as $attendance)
