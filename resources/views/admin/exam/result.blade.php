            ...
            <div class="row">
                                        ...
                                        <select class="form-control" name="type" id="type" required>
                                            ...
                                            @foreach ($types as $type)
                                                <option value="{{ $type->id }}"
                                                    @if ($selected_type == $type->id) selected @endif>{{ $type->title }}
                                                </option>
                                            @endforeach
                                        </select>
                ...
                <div class="col-sm-12">
                    <div class="card">
                        @if (isset($rows))
                            <div class="card-header">
                                <a href="{{ route('admin.exam-result') }}" class="btn btn-info"><i
                                            ...
                                            @foreach ($rows as $key => $row)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($row->attendance == 1)
                                                            <span
                                                                class="badge badge-primary">{{ __('attendance_present') }}</span>
                                                        @else
                                                            <span
                                                                class="badge badge-danger">{{ __('attendance_absent') }}</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if ($row->attendance == 1)
                                                    ...
                                                    <td>
                                                        ...
                                                        @else
                                                            <span
                                                                class="badge badge-danger">{{ __('status_failed') }}</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if ($row->attendance == 1)
...
@section('page_js')
    <script>
        $("#mark_category_types").on('change', function(e) {
            e.preventDefault(e);
            var type = $("#type");
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('filter-mark-types') }}",
                data: {
                    _token: $('input[name=_token]').val(),
                    mark_category_id: $(this).val()
                },
