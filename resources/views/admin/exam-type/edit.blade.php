                ...
                <form class="needs-validation" novalidate action="{{ route($route . '.update', $row->id) }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="title" class="form-label">{{ __('module_exam_type_category') }}
                                <span>*</span></label>
                            <select name="category" id="category" class="form-control" required>
                                <option value="">{{ __('select') }}</option>
                                @foreach ($mark_distribution_categories as $category)
                                    <option value="{{ $category->id }}" @selected($row->exam_type_category_id == $category->id)>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control autonumber" name="marks" id="marks"
                                value="{{ round($row->marks, 2) }}" required>
