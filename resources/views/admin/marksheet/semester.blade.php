                    ...
                    <div class="card">
                        ...
                        <div class="card-block">
                            <form class="needs-validation" novalidate method="get" action="{{ route($route . '.semester') }}">
                                <div class="row gx-2">
                                    <div class="form-group col-md-3 @isset($hide_program_filter) d-none @endisset">
                                        <label for="program">{{ __('field_program') }} <span>*</span></label>
                                        <select class="form-control program" name="program" id="program"
                                            @if (!isset($hide_program_filter)) required @endif>
                                            <option value="">{{ __('select') }}</option>
                                            @foreach ($programs as $program)
                                                <option value="{{ $program->id }}"
                                                    @if ($selected_program == $program->id) selected @endif>{{ $program->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                            ...
                                            @foreach ($rows as $key => $row)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @can($access . '-print')
                                                            @if (isset($print))
                                                                <a href="#" class="btn btn-icon btn-dark btn-sm"
                                                                    onclick="PopupWin('{{ route($route . '.semester.print', ['id' => $row->id, 'session' => $selected_session]) }}', '{{ $title }}', 1000, 600);">
                                                        ...
                                                        @can($access . '-download')
                                                            @if (isset($print))
                                                                <a href="{{ route($route . '.semester.download', ['id' => $row->id, 'session' => $selected_session]) }}"
