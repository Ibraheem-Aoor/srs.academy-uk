...
<body>
    ...
    <div class="template-container" id="downloadable"
        style="width: {{ $marksheet->width }}; height: {{ $marksheet->height }};">
        <div class="template-inner">
                        ...
                        <td class="temp-logo">
                            <div class="inner">
                                @if (is_file('uploads/' . $path . '/' . $marksheet->logo_left))
                                    <img src="{{ asset('uploads/' . $path . '/' . $marksheet->logo_left) }}"
                                        alt="Logo">
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="temp-logo last">
                            <div class="inner">
                                @if (is_file('uploads/' . $path . '/' . $marksheet->logo_right))
                                    <img src="{{ asset('uploads/' . $path . '/' . $marksheet->logo_right) }}"
                                        alt="Logo">
                                @endif
                            </div>
                        </td>
                    ...
                    @foreach ($item->subjects as $subject)
                        ...
                        @foreach ($grades as $grade)
                         @if ($marks_per >= $grade->min_mark && $marks_per <= $grade->max_mark)
                                @php
                                    if ($grade->point > 0) {
                                        $total_cgpa = $total_cgpa + $grade->point * $subject->credit_hour;
            ...
            <table class="table-no-border marksheet">
                ...
                @foreach ($semester_items as $key => $semester_item)
                    <tbody>
                        <tr>
                            <td colspan="5" class="semester-title">{{ $semester_item[0] }} |
                                {{ $semester_item[1] }} | {{ $semester_item[2] }}</td>
                        </tr>
