<div class="card-block" id="deleteRoutine-{{ $row->id }}">
    <div class="row">
        @isset($row)
            ...
            <input type="checkbox" id="delete_routine-{{ $row->id }}" name="delete_routine[]" value="{{ $row->id }}"
                hidden>
        @endisset
            ...
            <select class="form-control select2" name="subject[]" id="subject" required>
                ...
                @foreach ($subjects as $subject)
                    <option value="{{ $subject->id }}"
                        @isset($row) {{ $row->subject_id == $subject->id ? 'selected' : '' }} @endisset>
                        {{ $subject->code }} - {{ $subject->title }}</option>
                @endforeach
            </select>
            ...
            <select class="form-control select2" name="teacher[]" id="teacher" required>
                ...
                @foreach ($teachers as $teacher)
                    <option value="{{ $teacher->id }}"
                        @isset($row) {{ $row->teacher_id == $teacher->id ? 'selected' : '' }} @endisset>
                        {{ $teacher->staff_id }} - {{ $teacher->first_name }} {{ $teacher->last_name }}</option>
                @endforeach
            </select>
            ...
            <select class="form-control select2" name="room[]" id="room" required>
                ...
                @foreach ($rooms as $room)
                    <option value="{{ $room->id }}"
                        @isset($row) {{ $row->room_id == $room->id ? 'selected' : '' }} @endisset>
                        {{ $room->title }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-2">
            ...
            <input type="time" class="form-control time" name="start_time[]" id="start_time"
        ...
        <div class="form-group col-md-2">
            ...
            <input type="time" class="form-control time" name="end_time[]" id="end_time"
