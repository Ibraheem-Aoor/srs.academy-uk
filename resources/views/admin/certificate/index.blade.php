                                        ...
                                        <select class="form-control" name="batch" id="batch" required>
                                            ...
                                            @foreach ($batchs as $batch)
                                                <option value="{{ $batch->id }}"
                                                    @if ($selected_batch == $batch->id) selected @endif>{{ $batch->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select class="form-control" name="template" id="template" required>
                                            ...
                                            @foreach ($templates as $template)
                                                <option value="{{ $template->id }}"
                                                    @if ($selected_template == $template->id) selected @endif>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($certificate_generate == 1)
                                                            <span
                                                                class="badge badge-pill badge-primary">{{ __('status_generated') }}</span>
                                                        @else
                                                            <span
                                                                class="badge badge-pill badge-danger">{{ __('status_not_generated') }}</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        @else
                                                            @foreach ($certificates as $certificate)
                                                                @if ($certificate->student_id == $row->id)
                                                                    ...
                                                                    @can($access . '-print')
                                                                        @if (isset($certificate_template))
                                                                            <a href="#"
                                                                                ...
                                                                                onclick="PopupWin('{{ route($route . '.print', ['id' => $certificate->id]) }}', '{{ $title }}', 1000, 600);">
                                                                    ...
                                                                    @can($access . '-download')
                                                                        @if (isset($certificate_template))
                                                                            <a href="{{ route($route . '.download', ['id' => $certificate->id]) }}"
                                                                                target="_blank"
                                                                                class="btn btn-icon btn-dark btn-sm">
